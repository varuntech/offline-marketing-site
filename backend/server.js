...
// Trust proxy - required for Railway/Heroku/etc to get real IP addresses
...
app.post('/api/pilot-request', 
    ...
    async (req, res) => {
        ...
        try {
            ...
            // Send success response immediately after database save
            res.status(201).json({
                success: true,
            ...
            // Process email and CRM in background (don't block response)
            ...
            setImmediate(async () => {
                ...
                if (transporter) {
                    try {
                        ...
                        console.error('Error sending user confirmation email:', emailError.message);
                    }
                }
                ...
                if (transporter) {
                    try {
                        ...
                        console.error('Error sending internal notification email:', emailError.message);
                    }
                }
            ...
            // 5. Send success response immediately after database save
            // Don't wait for email/CRM since those can fail
            res.status(201).json({
                success: true,
                message: 'Pilot request submitted successfully',
                requestId: requestId
